-- patched: حذف local های بالای فایل برای کاهش upvalues
...
dkjson = dofile("./Lib/dkjson.lua");
serpent = dofile("./Lib/serpent.lua");
base = dofile("./Lib/redis.lua");
...
Plan1 = 2592000;
Plan2 = 7776000;
Bot_Api = "https://api.telegram.org/bot" .. JoinToken;
BASSE = "https://api.telegram.org/bot" .. JoinToken;
tdlib = require("tdlib");
tdlib.set_config({
	api_id = Config.ApiID,
...
TD = tdlib.get_functions();
Bot_iD, TOKNE_ID = string.match(JoinToken, "(%d+):(%S+)");
_ = {
	process = 0,
	auto_run = 0
};
...
is_filter = function(msg, value)
	list = base:smembers(TD_ID .. "Filters:" .. msg.chat_id);
	var = false;
	for i = 1, #list do
...
replace = function(value, del, find)
	del = del:gsub("[%(%)%.%+%-%*%?%[%]%^%$%%]", "%%%1");
	find = find:gsub("[%%]", "%%%%");
	return string.gsub(value, del, find);
end;
sendBot = function(chat_id, reply_to_message_id, text, parse_mode, callback, data)
...
SendInlineBot = function(chat_id, text, keyboard, markdown)
	url = Bot_Api;
	if keyboard then
...
changeChatDescription = function(chat_id, des)
	bot = Config.JoinToken;
	url = "https://api.telegram.org/bot" .. bot;
	t = url .. "/setChatDescription?chat_id=" .. chat_id .. "&description=" .. des;
	return https.request(t);
end;
MuteUser = function(chat_id, user_id, time)
	Rep = Bot_Api .. "/restrictChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_post_messages=false&until_date=" .. time;
	return https.request(Rep);
end;
kickChatMemberTime = function(chat_id, user_id, time)
	Rep = Bot_Api .. "/kickChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&until_date=" .. time;
	return https.request(Rep);
end;
KickUser = function(chat_id, user_id)
	Rep = Bot_Api .. "/kickChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id;
	return https.request(Rep);
end;
UnRes = function(chat_id, user_id)
	Rep = Bot_Api .. "/restrictChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_post_messages=true&can_add_web_page_previews=true&can_send_other_messages=true&can_send_media_messages=true";
	return https.request(Rep);
end;
MuteUser2 = function(chat_id, user_id, time)
	Rep = Bot_Api .. "/restrictChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_send_media_messages=false&until_date=" .. time;
	return https.request(Rep);
end;
MuteMedia = function(chat_id, user_id)
	Rep = Bot_Api .. "/restrictChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_post_messages=true&can_add_web_page_previews=true&can_send_other_messages=false&can_send_media_messages=false";
	return https.request(Rep);
end;
MuteMediaTime = function(chat_id, user_id, time)
	Rep = Bot_Api .. "/restrictChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_post_messages=true&can_add_web_page_previews=true&can_send_other_messages=false&can_send_media_messages=false&until_date=" .. time;
	return https.request(Rep);
end;
SetAdmins = function(chat_id, user_id)
	Rep = Bot_Api .. "/promoteChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_change_info=true&can_pin_messages=true&can_restrict_members=true&can_invite_users=true&can_delete_messages=true";
	return https.request(Rep);
end;
membersCount = function(chat_id)
	Rep = Bot_Api .. "/getChatMembersCount?chat_id=" .. chat_id;
	return https.request(Rep);
end;
Forward = function(chat_id, from_chat_id, message_id)
	url = Bot_Api .. "/forwardMessage?chat_id=" .. chat_id .. "&from_chat_id=" .. from_chat_id .. "&message_id=" .. message_id;
	return https.request(url);
end;
remadmins = function(chat_id, user_id)
	Rep = Bot_Api .. "/promoteChatMember?chat_id=" .. chat_id .. "&user_id=" .. user_id .. "&can_change_info=false&can_pin_messages=false&can_restrict_members=false&can_invite_users=false&can_delete_messages=false";
	return https.request(Rep);
end;
sendMedia = function(chat_id, file_id, reply_to_message_id, caption, parse_mode, media)
...
reportowner = function(text)
	if reportpv then
		data = (TD.getChatAdministrators(chat_id)).administrators;
...
getTimeUptime = function(seconds, lang)
...
getTimeUptime2 = function(seconds, lang)
	days = math.floor(seconds / 86400);
	seconds = seconds - days * 86400;
	hours = math.floor(seconds / 3600);
	seconds = seconds - hours * 60 * 60;
...
isLeaderBots = function(msg)
	var = false;
	if msg.sender_id.user_id == 935728510 or msg.sender_id.user_id == 5126622470 or msg.sender_id.user_id == Config.Leader then
		var = true;
	end;
	return var;
end;
isLeaderBots1 = function(user)
	var = false;
	if user == 935728510 or user == 5126622470 then
		var = true;
	end;
	return var;
end;
is_configure = function(msg)
	var = false;
	if msg.sender_id.user_id == 935728510  or msg.sender_id.user_id == Config.Leader or msg.sender_id.user_id == 5126622470 or isLeaderBots(msg) then
		var = true;
	end;
	return var;
end;
is_FullSudo = function(msg)
	var = false;
...
is_OwnerPlusChat = function(msg)
	hash = base:sismember(TD_ID .. "OwnerListPlus:" .. msg.chat_id, msg.actor_user_id);
...
is_OwnerChat = function(msg)
	hash = base:sismember(TD_ID .. "OwnerList:" .. msg.chat_id, msg.actor_user_id);
	if hash or is_SudoChat(msg) then
		return true;
	else
		return false;
...
is_ModChat = function(msg)
	hash = base:sismember(TD_ID .. "ModList:" .. msg.chat_id, msg.actor_user_id);
...
is_VipChat = function(msg)
	hash = base:sismember(TD_ID .. "Vip:" .. msg.chat_id, msg.actor_user_id);
...
FullUser = function(user_id)
	user_id = user_id or 0;
	Full = Config.Full_Sudo;
	if Full then
		return true;
	else
		return false;
...
is_PrivateMemeber = function(user_id)
	var = false;
	hash = "PrivateMemeber:";
	PrivateMemeber = base:sismember(hash, user_id);
	if PrivateMemeber then
		var = true;
	end;
	return var;
end;
is_PrivateMem = function(msg, user_id)
	user_id = user_id or 0;
	PrivateMem = base:sismember("PrivateMemeber:", user_id);
	if PrivateMem then
		return true;
	else
		return false;
...
SudUser = function(msg, user_id)
	user_id = user_id or 0;
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if Sudo then
		return true;
	else
		return false;
...
is_OwnerPlus = function(msg)
	hash = base:sismember(TD_ID .. "OwnerListPlus:" .. msg.chat_id, msg.sender_id.user_id);
...
OwnUserPlus = function(msg, user_id)
	user_id = user_id or 0;
	OwnerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. msg.chat_id, user_id);
	if OwnerPlus then
		return true;
	else
		return false;
...
is_Owner = function(msg)
	hash = base:sismember(TD_ID .. "OwnerList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) then
		return true;
	else
		return false;
...
OwnUser = function(msg, user_id)
	user_id = user_id or 0;
	Owner = base:sismember(TD_ID .. "OwnerList:" .. msg.chat_id, user_id);
	if Owner then
		return true;
	else
		return false;
...
is_Nazem = function(msg)
	hash = base:sismember(TD_ID .. "NazemList:" .. msg.chat_id, msg.sender_id.user_id);
...
NazemUser = function(msg, user_id)
	user_id = user_id or 0;
	Owner = base:sismember(TD_ID .. "NazemList:" .. msg.chat_id, user_id);
	if Owner then
		return true;
	else
		return false;
...
is_Mod = function(msg)
	hash = base:sismember(TD_ID .. "ModList:" .. msg.chat_id, msg.sender_id.user_id);
...
ModUser = function(msg, user_id)
	user_id = user_id or 0;
	Mod = base:sismember(TD_ID .. "ModList:" .. msg.chat_id, user_id);
	if Mod then
		return true;
	else
		return false;
...
is_ModTest = function(msg)
	hash = base:sismember(TD_ID .. "ModListtest:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
		return true;
	else
		return false;
...
ModUserTest = function(msg, user_id)
	user_id = user_id or 0;
	Mod = base:sismember(TD_ID .. "ModListtest:" .. msg.chat_id, user_id);
	if Mod then
		return true;
	else
		return false;
...
is_Vip = function(msg)
	hash = base:sismember(TD_ID .. "Vip:" .. msg.chat_id, msg.sender_id.user_id);
...
VipUser = function(msg, user_id)
	user_id = user_id or 0;
	vip = base:sismember(TD_ID .. "Vip:" .. msg.chat_id, user_id);
	if vip then
		return true;
	else
		return false;
...
is_GlobalyBan = function(user_id)
	var = false;
	hash = "GlobalyBanned:";
	gbanned = base:sismember(hash, user_id);
	if gbanned then
		var = true;
	end;
	return var;
end;
is_GlobalyMute = function(user_id)
	var = false;
	hash = "AGTMute:";
	gbanned = base:sismember(hash, user_id);
	if gbanned then
...
is_ModClean = function(msg)
	hash = base:sismember(TD_ID .. "ModCleanList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
		return true;
	else
		return false;
...
is_ModClean = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModCleanList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	...
	elseif base:sismember(TD_ID .. "ModCleanList:" .. chat_id, user_id) then
		...
		var = false;
		results = TD.getUser(user_id);
		ID = "<a href=\"tg://user?id=" .. user_id .. "\"> " .. results.first_name .. "</a>";
...
Charge = function(chat_id, user_id)
	var = false;
...
Banall = function(chat_id, user_id)
	var = false;
	if tonumber(user_id) == Sudoid then
		var = true;
...
KickAll = function(chat_id, user_id)
	var = false;
	if tonumber(user_id) == Sudoid then
		var = true;
...
is_ModBan = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModBanList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModMute = function(msg)
	hash = base:sismember(TD_ID .. "ModMuteList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
		return true;
	else
		return false;
...
is_ModMute = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModMuteList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModRid = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModRidBot:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModWarn = function(msg)
	hash = base:sismember(TD_ID .. "ModWarnList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
		return true;
	else
		return false;
...
is_ModWarn = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModWarnList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModLock = function(msg)
	hash = base:sismember(TD_ID .. "ModLockList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
		return true;
	else
		return false;
...
is_ModLock = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModLockList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModVipCmd = function(msg)
	hash = base:sismember(TD_ID .. "ModVipList:" .. msg.chat_id, msg.sender_id.user_id);
...
is_ModVipCmd = function(chat_id, user_id)
	var = false;
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModVipList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	if tonumber(user_id) == Sudoid or owner or ownerPlus or Sudo or nazem then
		var = true;
...
is_ModPanelCmd = function(msg)
	hash = base:sismember(TD_ID .. "ModpanelList:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Sudo(msg) or is_Owner(msg) or is_OwnerPlus(msg) then
		return true;
	else
		return false;
...
is_ModPanelCmd = function(chat_id, user_id)
	var = false;
	for v, user in pairs(SUDO) do
	...
	nazem = base:sismember(TD_ID .. "NazemList:" .. chat_id, user_id);
	owner = base:sismember(TD_ID .. "OwnerList:" .. chat_id, user_id);
	hash = base:sismember(TD_ID .. "ModpanelList:" .. chat_id, user_id);
	Sudo = base:sismember(TD_ID .. "SUDO", user_id);
	OwnerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. chat_id, user_id);
...
is_VipJoin = function(msg)
	hash = base:sismember(TD_ID .. "VipJoin:" .. msg.chat_id, msg.sender_id.user_id);
	if hash or is_Mod(msg) then
		return true;
	else
		return false;
...
is_Banned = function(chat_id, user_id)
	hash = base:sismember(TD_ID .. "BanUser:" .. chat_id, user_id);
	if hash then
		return true;
	else
		return false;
...
is_MuteUser = function(chat_id, user_id)
	hash = base:sismember(TD_ID .. "MuteUser:" .. chat_id, user_id);
	if hash then
		return true;
	else
		return false;
...
is_JoinChannel = function(msg)
	if base:get(TD_ID .. "joinchnl") then
		url = https.request("https://api.telegram.org/bot" .. JoinToken .. "/getchatmember?chat_id=@" .. Config.Channel .. "&user_id=" .. msg.sender_id.user_id);
		if res ~= 200 then
		end;
		...
		if not is_GlobalyBan(msg.sender_id.user_id) and (not Joinchanel.ok or Joinchanel.result.status == "left" or Joinchanel.result.status == "kicked") and (not is_Sudo(msg)) then
			...
			elseif result.first_name ~= "" then
				...
				ID = "<a href=\"tg://user?id=" .. msg.sender_id.user_id .. "\">" .. StringData(result.username) .. "</a>";
			end;
...
function DisplayID(msg, chat, user)
	...
	elseif result.first_name ~= "" then
		...
		usernamelink = "<a href=\"tg://user?id=" .. user .. "\">" .. result.username .. "</a>";
	end;
	data = TD.getUserProfilePhotos(user, 0, 1);
	Profile = (TD.getUserProfilePhotos(user, 0, 1)).total_count;
	...
	Message = tonumber(base:get(TD_ID .. "All:Message:" .. chat)) or 0;
	getUseraddDay_ = tonumber(base:get(TD_ID .. "Content_Message:AddsDay:" .. user .. ":" .. chat)) or 0;
	getUseradd = tonumber(base:get(TD_ID .. "Content_Message:Adds:" .. user .. ":" .. chat)) or 0;
	getUserday = tonumber(base:get(TD_ID .. "Content_Message:MsgsDay:" .. user .. ":" .. chat)) or 0;
	getUserMem = tonumber(base:get(TD_ID .. "Content_Message:Msgs:" .. user .. ":" .. chat)) or 0;
	...
	if tonumber(user) == tonumber(935728510) or tonumber(user) == tonumber(5126622470) then
		rank = "توسعه دهنده";
	...
	t = "اطلاعات کاربر";
	r = "<a href=\"tg://user?id=" .. user .. "\"> " .. t .. "</a>";
	r1 = "<a href=\"tg://user?id=" .. user .. "\"> آمار کاربر</a>";
...
lock_del = function(msg, fa)
	TD.deleteMessages(msg.chat_id, {
		[1] = msg.id
	});
	Rosh = TD.getUser(msg.sender_id.user_id);
	if Rosh.username == "" then
		name = ec_name(Rosh.first_name);
	else
		name = Rosh.username;
	end;
	url = https.request(Bot_Api .. "/getChatMember?chat_id=" .. msg.chat_id .. "&user_id=" .. BotJoiner);
	if res ~= 200 then
	end;
	...
	if statsurl.ok == true and statsurl.result.status == "administrator" and statsurl.result.can_restrict_members == true then
		if not base:sismember((TD_ID .. "Gp2:" .. msg.chat_id), "MsgCheckPm") then
			...
			if not base:get((TD_ID .. "TimerDel:" .. msg.sender_id.user_id .. msg.chat_id)) then
				...
				if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "GozareshDel") then
					data = (TD.getChatAdministrators(msg.chat_id)).administrators;
					for m, n in ipairs(data) do
						if n.user_id then
							if n.is_owner == true then
								owner_id = n.user_id;
		...
		username = "<a href=\"tg://user?id=" .. msg.sender_id.user_id .. "\"> " .. name .. "</a> ";
		sendBot(msg.chat_id, msg.id, "⫸ کاربر : <b>" .. username .. "</b> \n◂ " .. fa .. " را انجام دادہ است ولی ربات  دسترسی کامل حذف کردن ندارد !", "html");
	end;
end;
lock_warn = function(msg, fa)
	hashwarnbd = TD_ID .. msg.sender_id.user_id .. ":warn";
	warnhashbd = base:hget(hashwarnbd, msg.chat_id) or 1;
	max_warn = tonumber(base:get(TD_ID .. "max_warn:" .. msg.chat_id) or 5);
	TD.deleteMessages(msg.chat_id, {
		[1] = msg.id
	});
	Rosh = TD.getUser(msg.sender_id.user_id);
		...
		if statsurl.ok == true and statsurl.result.status == "administrator" and statsurl.result.can_restrict_members == true then
			if not base:sismember((TD_ID .. "Gp2:" .. msg.chat_id), "MsgCheckPm") then
				...
				if tonumber(1) > tonumber(totalpms) then
					totalpmsmrr = tonumber(totalpms) + 1;
					username = "<a href=\"tg://user?id=" .. msg.sender_id.user_id .. "\">" .. StringData(alpha.first_name) .. "</a>";
...
function lock_mute(msg, fa)
	base:sadd(TD_ID .. "DelUser" .. msg.chat_id, msg.sender_id.user_id);
	TD.deleteMessages(msg.chat_id, {
		[1] = msg.id
	});
	function BDDelUser()
		timemutemsg = tonumber(base:get(TD_ID .. "mutetime:" .. msg.chat_id) or 3600);
		alpha = TD.getUser(msg.sender_id.user_id);
		if alpha.username == "" then
			name = ec_name(alpha.first_name);
		else
		...
		if statsurl.ok == true and statsurl.result.status == "administrator" and statsurl.result.can_restrict_members == true then
			if not base:sismember((TD_ID .. "Gp2:" .. msg.chat_id), "MsgCheckPm") then
				...
				ex = base:get(TD_ID .. "mutetime:" .. msg.chat_id);
				totalpms = base:get(TD_ID .. "pmdadeshode" .. msg.chat_id .. msg.sender_id.user_id .. os.date("%Y/%m/%d")) or 0;
				if tonumber(1) > tonumber(totalpms) then
					totalpmsmrr = tonumber(totalpms) + 1;
					username = "<a href=\"tg://user?id=" .. msg.sender_id.user_id .. "\"> " .. name .. "</a> ";
		...
		if statsurl.ok == true and statsurl.result.status == "administrator" and statsurl.result.can_restrict_members == true then
			if not base:sismember((TD_ID .. "Gp2:" .. msg.chat_id), "MsgCheckPm") then
				if base:get(TD_ID .. "ban_stats" .. msg.chat_id) == "bantime" then
					ex = tonumber(base:get(TD_ID .. "bantime:" .. msg.chat_id) or 3600);
					Time_ = getTimeUptime(ex);
...
function MsgCheck(msg, fa, Redis, Redis2)
...
function checkerclean()
	if _.auto_run == 2 then
		TD.set_timer(10, checkerclean);
		ListGroup = base:smembers(TD_ID .. "group:");
		if #ListGroup ~= 0 then
		...
		if #ListGroup ~= 0 then
			for a, b in pairs(ListGroup) do
				...
				if timeresetstats or timeresetstats then
					...
					if Time == Start or Time >= Start and Time < End or Start > End and (Time >= Start and Time <= 2359 or Time < End and Time >= 0) then
						if not base:sismember((TD_ID .. "Gp2:" .. b), "timeresetGp") then
							...
							ListMember = base:smembers(TD_ID .. "Sender_user_ids:" .. b);
				...
				if base:get(TD_ID .. "atolct2" .. b) or base:get(TD_ID .. "atolct2" .. b) then
					...
					elseif tonumber(Time) >= tonumber(End) then
						if base:get(TD_ID .. "bot:muteall:Run" .. b) then
							result = (TD.getChat(b)).permissions;
				...
				if not base:get((TD_ID .. "cgmauto:" .. b)) and base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoon") then
					timecgm = base:get(TD_ID .. "cgmautotime:" .. b);
				...
				if base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoalarm") then
					if not base:get((TD_ID .. "cgmauto1:" .. b)) and base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoon") then
						timecgm = base:get(TD_ID .. "cgmautotime1:" .. b);
						base:setex(TD_ID .. "cgmauto1:" .. b, timecgm, true);
						timecgmbaghi = base:ttl(TD_ID .. "cgmauto:" .. b);
						Time_ = getTimeUptime(timecgmbaghi);
					...
					if not base:get((TD_ID .. "cgmauto2:" .. b)) and base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoon") then
						timecgm = base:get(TD_ID .. "cgmautotime2:" .. b);
					...
					if not base:get((TD_ID .. "cgmauto3:" .. b)) and base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoon") then
						timecgm = base:get(TD_ID .. "cgmautotime3:" .. b);
					...
					if not base:get((TD_ID .. "cgmauto4:" .. b)) and base:sismember(TD_ID .. "Gp2:" .. b, "cgmautoon") then
						timecgm = base:get(TD_ID .. "cgmautotime4:" .. b);
...
base:setex(TD_ID .. "teletab:" .. msg.chat_id, 3600, true);
		...
		if not base:get((TD_ID .. "timestatsreer:" .. msg.chat_id)) then
			ListMember = base:smembers(TD_ID .. "Sender_user_ids:" .. msg.chat_id);
			if #ListMember == 0 then
			else
				for i, i in pairs(ListMember) do
...
if Ramin.username == "" then name = ec_name(Ramin.first_name) 
else 
name = Ramin.username;
end;
	...
	elseif base:get(TD_ID .. "tabchi_stats" .. msg.chat_id) == "off" then
			...
			if base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "forceadd") then
				if not is_Vip(msg) and (not base:sismember((TD_ID .. "VipAdd:" .. msg.chat_id), msg.sender_id.user_id)) then
					...
					if tonumber(permit) < tonumber(setadd) then
						...
						if TeleBot.username == "" then
							name = ec_name(TeleBot.first_name);
						else
							name = TeleBot.username;
						end;
		...
		elseif base:sismember(TD_ID .. "Gp2:" .. msg.chat_id, "forcejoin") and (not (msg.content["@type"] == "messageChatJoinByLink" or msg.content["@type"] == "messageChatAddMembers" or msg.content["@type"] == "messageChatDeleteMember")) then
			...
			if not is_GlobalyBan(msg.sender_id.user_id) and (not Joinchanel.ok or Joinchanel.result.status == "left" or Joinchanel.result.status == "kicked") and (not is_Sudo(msg)) and (not is_Owner(msg)) and (not is_OwnerPlus(msg)) and (not base:sismember((TD_ID .. "VipAdd:" .. msg.chat_id), msg.sender_id.user_id)) and (not is_Vip(msg)) then
				...
				local Rosha = replace(Rosha, "USERNAME", "@" .. TeleBot.username) or "بدون@";
			...
			if is_Owner(msg) or is_OwnerPlus(msg) or is_Nazem(msg) then
				...
				if Ramin == "لیست لقب مدیران" or Ramin == "adminranks" then
					...
					if #data.administrators ~= 0 or #data.administrators ~= 1 then
						...
						for i, v in pairs(data.administrators) do
							if not v.is_owner then
								name = (TD.getUser(v.user_id)).username or (TD.getUser(v.user_id)).first_name;
...
sendBot(msg.chat_id,msg.id,'⌯ زبان ربات به فارسی می باشد .','md')
		...
		local function GroupStatsAdmin(chat_id, msg_id, counter)
			base:srem(TD_ID .. "Sender_user_ids:" .. chat_id, Bot_iD);
		...
		--Username: '..(tdlib_functions.getMe().username ~= '' and '@'..tdlib_functions.getMe().username or '---')..'
		--<a href=\"tg://user?id=" .. BotJoiner .. "\">" .. (TD.getMe()).username .. "</a>
		...
		if Ramin == "ربات" or Ramin == "bot" and is_Mod(msg) then
			if msg.sender_id.user_id == 724990027 then
				...
				if Bot then
					...
					if tonumber(user) == tonumber(935728510) or tonumber(user) == tonumber(935728510) then
						rankuser = "👑 پادشاه ";
					elseif tonumber(user) == tonumber(Config.Sudoid) then
						rankuser = "🤠 بابایی ";
					elseif SudUser(msg, user) then
			...
			for k, v in pairs(results.members) do
				if tonumber(v.member_id.user_id) ~= tonumber(Sudoid) then
					...
					elseif result.first_name ~= "" then
						...
						username = StringData(result.username);
					end;
					...
					base:set(TD_ID .. "FirstName:" .. v.member_id.user_id, StringData(result.username));
				end;
			end;
		end;
				...
				if n.user_id then
					if n.is_owner == true then
						...
						elseif result.first_name ~= "" then
							...
							usernamelink = "<a href=\"tg://user?id=" .. owner_id .. "\">" .. StringData(result.username) .. "</a>";
						end;
						local data = TD.getUserProfilePhotos(owner_id, 0, 1);
						local Profile = (TD.getUserProfilePhotos(owner_id, 0, 1)).total_count;
						if result.first_name then
	...
	if ChatTypeSuperGp and not ChatTypeChannel and not ChatTypePV then
		...
		if Userdelban and (not is_OwnerChat(msg)) then
			...
			if TeleBot.type["@type"] == "userTypeBot" then
				...
				if Azader.username == "" then
					name = ec_name(Azader.first_name);
				else
					name = Azader.username;
				end;
				...
				elseif Owner.first_name ~= "" then
					...
					Owner = "<a href=\"tg://user?id=" .. owner_id .. "\">" .. StringData(Owner.username) .. "</a>";
				end;
		...
		if Usernewjoin or Usernewadd or Useraddban then
			...
			if base:sismember(TD_ID .. "Gp2:" .. ChatID, "Welcomeon") then
				...
				local txtt = replace(txtt, "username", "@" .. Ramin.username) or "nil";
				if Media == "photo" then
		...
		if Usernewjoin or Usernewadd then
			...
			if UserCheck == Config.BotJoiner then
				...
				elseif TeleBot.first_name ~= "" then
					...
					username = "<a href=\"tg://user?id=" .. msg.actor_user_id .. "\">" .. StringData(TeleBot.username) .. "</a>";
				end;
				local title = (TD.getChat(msg.chat_id)).title;
			...
			if user == tonumber(BotJoiner) then
				if base:get(TD_ID .. "freeinstall") then
					...
					elseif TeleBot.first_name ~= "" then
						...
						username = "<a href=\"tg://user?id=" .. inviteruser .. "\">" .. StringData(TeleBot.username) .. "</a>";
					end;
...
local CheckMasssageBot = function(msg, data)
	...
	if Ramin == "Myposition." or Ramin == "من کیم" or Ramin == "مقام من" then
		if tonumber(msg.sender_id.user_id) == tonumber(935728510) or tonumber(msg.sender_id.user_id) == tonumber(5126622470) then
			rank = "توسعه دهنده";
...
local UpdateCallback = function(msg, data)
	if msg.chat_instance and msg.payload["@type"] == "callbackQueryPayloadData" then
		base:setex(TD_ID .. "PanelAutoCall:" .. msg.chat_id, 60, true);
		base:setex(TD_ID .. "TimerPanel:" .. msg.sender_user_id .. msg.chat_id, 60, true);
...
local BotGetQuery = function(msg, data)
	...
	local is_Mod = function(chat_id, user_id)
		...
		local owner = base:sismember(TD_ID .. "OwnerList:" .. msg.chat_id, msg.sender_user_id);
		local ownerPlus = base:sismember(TD_ID .. "OwnerListPlus:" .. msg.chat_id, msg.sender_user_id);
		local nazem = base:sismember(TD_ID .. "NazemList:" .. msg.chat_id, msg.sender_user_id);
	...
	Click = tonumber(base:get(TD_ID .. "CheckClick:" .. msg.sender_user_id)) or 0
	if Click > 6 then
		base:del(TD_ID .. "CheckClick:" .. msg.sender_user_id)
		base:setex(TD_ID .. "LimitedClick3:" .. msg.sender_user_id, 3, true)
		if rank(msg.sender_user_id, msg.chat_id) <= 4 then
		  ...
		  return TD.setChatMemberStatus(msg.chat_id, msg.sender_user_id, "banned")
		end
	  end
	  if base:get(TD_ID .. "LimitedClick3:" .. msg.sender_user_id) then
		if rank(msg.sender_user_id, msg.chat_id) <= 4 then
	  ...
	  base:setex(TD_ID .. "CheckClick:" .. msg.sender_user_id, 3, Click + 1)
	 
	  -- if result and (result.sender_id.user_id or msg.sender_user_id) ~= msg.sender_user_id then
		  -- TD.answerCallbackQuery(msg.id, " پنل مدیریتی از شما فرمان نمی گیرد !", true);
		  -- return;
	  -- end; 
	  -- if not is_Mod(msg.chat_id, msg.sender_user_id) then
		-- inlineeqq = base:get(TD_ID .. "antiattackinline:" .. msg.sender_user_id .. ":" .. msg.chat_id) or 0;
		-- if tonumber(inlineeqq) < 6 then
			-- base:setex(TD_ID .. "antiattackinline:" .. msg.sender_user_id .. ":" .. msg.chat_id, 60, inlineeqq + 1);
	...
	if callback and string.match(callback, "^actevemem$") then
		...
		local TarikhEN = "📆 امروز " .. roz .. " " .. tarikhnumberen .. "\n⏰ ساعت : " .. timeen .. "";
		local TarikhFA = "📆 امروز " .. roz .. " " .. tarikhnumberfa .. "\n⏰ ساعت : " .. timefa .. "";
		base:srem(TD_ID .. "Sender_user_ids:" .. msg.chat_id, BotJoiner);
			local msgs, msgsday, adds, addsday = {}, {}, {}, {};
			local function getNum(data, rank, status)
				if data == "msgs" then
					do
						do
							for k, i in pairs(base:smembers(TD_ID .. "_Sender_user_ids:" .. msg.chat_id)) do
								...
								if tonumber(getUser) == tonumber(msgs[(#msgs)]) then
									...
									table.remove(msgs, #msgs);
									base:srem(TD_ID .. "_Sender_user_ids:" .. msg.chat_id, i);
									break;
								end;
							end;
						end;
					...
					if status == "msgs" then
						base:del(TD_ID .. "_Sender_user_ids:" .. msg.chat_id);
					end;
				end;
			end;
			do
				do
					for k, i in pairs(base:smembers(TD_ID .. "Sender_user_ids:" .. msg.chat_id)) do
						base:sadd(TD_ID .. "_Sender_user_ids:" .. msg.chat_id, i);
						base:sadd(TD_ID .. "_Sender_user_idsd:" .. msg.chat_id, i);
						base:sadd(TD_ID .. "Sender_user_ids_:" .. msg.chat_id, i);
			...
			counter = 5;
			if counter then
				local members = #base:smembers((TD_ID .. "Sender_user_ids:" .. msg.chat_id));
				if members < tonumber(counter) then
					_c = members;
				else
					_c = tonumber(counter);
